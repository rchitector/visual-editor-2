...
<template>
    <div v-if="canvasStore.debug && props.width > 0 && props.height > 0 "
    ...
    >
        ...
        >{{props.text}}: (x:{{props.x.toFixed(2)}} y:{{props.y.toFixed(2)}} width:{{props.width.toFixed(2)}} height:{{props.height.toFixed(2)}})</div>
    </div>
    <div v-if="canvasStore.debug"
         :style="{
        ...
        >{{props.text}}: center (x:{{((props.x * 2 + props.width) / 2).toFixed(2)}} y:{{((props.y*2 + props.height) / 2).toFixed(2)}})</div>
    </div>
