...
import DataIcon from "@/js/components/Icons/DataIcon.vue";
...
<template>
    <div class="relative port whitespace-nowrap">
        <div>{{ props.title }}</div>
        <div v-if="props.type == PortType.DataInput"
             @mousedown="onInputPortPointDown"
             class="absolute top-1 -left-7 pointer-events-auto">
            <DataIcon :active="props.active" :disabled="props.disabled" :baseColor="props.baseColor"/>
        </div>
        <div v-if="props.type == PortType.DataOutput"
