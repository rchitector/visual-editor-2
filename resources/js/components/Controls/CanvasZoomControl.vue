...
<template>
    <div class="absolute right-5 bottom-5 rounded-lg border border-gray-200 dark:border-gray-700 p-2 bg-white dark:bg-gray-800">

        <div class="gap-1 flex items-center">
            <button @click="()=>store.zoomOut()"
            ...
            <div id="dropdownTop" class="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownTopButton">
                    <li>
                        <button @click="store.zoomFit" type="button" class="w-full block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white flex items-center gap-2">
                    ...
                    <li v-for="level in ZOOM_VISIBLE_LEVELS" :key="level">
                        <button @click="()=>store.setZoom(level)" type="button"
            ...
            <button @click="()=>store.zoomIn()" type="button"
